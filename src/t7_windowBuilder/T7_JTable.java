...
public class T7_JTable extends JFrame{
	...
	public T7_JTable() {
		...
		table.getColumnModel().getColumn(0).setMaxWidth(50); // 컬럼의 최대 크기 지정
...
//					JOptionPane.showMessageDialog(null, "선택된 행 인덱스 번호 : "+row);
//				}
				...
				if(row != -1) {
					...
					// 선택된 행의 모든 정보를 메세지 창에 띄우시요.
					
					InsaVO vo = dao.getNameSearch(table.getValueAt(row, 1)+"");
					String temp = "성명:"+vo.getName()+", 나이"+vo.getAge();
					System.out.println(temp);
					/*
					String msgTemp = "";
					for(int i=0 ; i<tcm.getColumnCount(); i++) {
						msgTemp += table.getValueAt(row, i) + " ";
					}
					System.out.println("선택된 내용 : " + msgTemp);
					lblMessage.setText("선택된 내용 : " + msgTemp);
					*/
				}
			}
		});
